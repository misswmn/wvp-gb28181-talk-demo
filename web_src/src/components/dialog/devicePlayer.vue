    ...
    <el-dialog title="视频播放" top="0" :close-on-click-modal="false" :visible.sync="showVideoDialog" @close="close()">
      <div style="width: 100%; height: 100%">
        <el-tabs type="card" :stretch="true" v-model="activePlayer" @tab-click="changePlayer"
          ...
          <el-tab-pane label="Jessibuca" name="jessibuca">
            <jessibucaPlayer v-if="activePlayer === 'jessibuca'" ref="jessibuca" :visible.sync="showVideoDialog"
              :videoUrl="videoUrl" :error="videoError" :message="videoError" height="100px" :hasAudio="hasAudio" fluent
              autoplay live></jessibucaPlayer>
          </el-tab-pane>
          <el-tab-pane label="WebRTC" name="webRTC">
            <rtc-player v-if="activePlayer === 'webRTC'" ref="webRTC" :visible.sync="showVideoDialog" :videoUrl="videoUrl"
              :error="videoError" :message="videoError" height="100px" :hasAudio="hasAudio" fluent autoplay
              live></rtc-player>
          </el-tab-pane>
        ...
        <jessibucaPlayer v-if="Object.keys(this.player).length == 1 && this.player.jessibuca" ref="jessibuca"
          :visible.sync="showVideoDialog" :videoUrl="videoUrl" :error="videoError" :message="videoError" height="100px"
      ...
      <div id="shared" style="text-align: right; margin-top: 1rem;">
        <el-tabs v-model="tabActiveName" @tab-click="tabHandleClick">
              ...
              <div class="control-panel">
                <el-button-group>
                  ...
                  <el-button style="position: absolute; left: 11rem; top: 2.5rem; width: 5rem" size="mini"
                    icon="el-icon-loading" @click="setSpeedOrTime(134, cruisingGroup, cruisingSpeed)">设置</el-button>
                  <el-tag style="position :absolute; left: 16rem; top: 2.5rem; width: 5rem; text-align: center"
                    size="medium">停留时间</el-tag>
                  ...
                  <el-button style="position: absolute; left: 27rem; top: 2.5rem; width: 5rem" size="mini"
                    icon="el-icon-timer" @click="setSpeedOrTime(135, cruisingGroup, cruisingTime)">设置</el-button>
                  <el-tag style="position :absolute; left: 0rem; top: 4.5rem; width: 5rem; text-align: center"
                    size="medium">巡航组编号</el-tag>
                  ...
                  <el-button style="position: absolute; left: 11rem; top: 9rem; width: 5rem" size="mini"
                    icon="el-icon-d-arrow-left" @click="setCommand(137, scanGroup, 1)">左边界</el-button>
                  <el-button style="position: absolute; left: 16rem; top: 9rem; width: 5rem" size="mini"
                    icon="el-icon-d-arrow-right" @click="setCommand(137, scanGroup, 2)">右边界</el-button>
                  <el-button style="position: absolute; left: 27rem; top: 7rem; width: 5rem" size="mini" type="primary"
                    icon="el-icon-video-camera-solid" @click="setCommand(137, scanGroup, 0)">扫描</el-button>
                  <el-button style="position: absolute; left: 27rem; top: 9rem; width: 5rem" size="mini" type="danger"
                    icon="el-icon-switch-button" @click="ptzCamera('stop')">停止</el-button>
                </el-button-group>
              </div>
            </div>
          </el-tab-pane>
          ...
          <el-tab-pane style="margin-left: 50%;" label="语音对讲" name="talkback">
            <div class="talk-div" @click="talkHandle">
              <i :class="'el-icon-microphone'" :title="talkTitle"
                :style="{ color: microphoneStatus ? 'rgba(25, 246, 5, 0.69)' : 'rgba(118, 118, 118, 0.742)' }"
                v-if="canTalk"></i>

              <i :class="'el-icon-turn-off-microphone'" :title="talkTitle" v-if="!canTalk"></i>
            </div>
          </el-tab-pane>
...
export default {
  ...
  computed: {
    ...
    canTalk() {
      ...
      if (!ishttps || !this.hasAudio) {
        this.talkTitle = "由于浏览器安全策略本站必须得是https，并且开启播放音频才可以对讲";
        return false;
      }
      this.talkTitle = "开始对讲";
      return true;
  ...
  methods: {
    ...
    startTalk() {
      ...
      this.$axios({
        method: 'get',
        url: `/api/talkback/getWebRtcAddr/${this.deviceId}/${this.channelId}`
      }).then(res => {
        ...
        _that.zlmRTCClient.on(ZLMRTCClient.Events.WEBRTC_ICE_CANDIDATE_ERROR, function (e) {// ICE 协商出错
          console.log('ICE 协商出错')
        });

        _that.zlmRTCClient.on(ZLMRTCClient.Events.WEBRTC_ON_REMOTE_STREAMS, function (e) {//获取到了远端流，可以播放
          console.log('播放成功', e.streams)
        });

        _that.zlmRTCClient.on(ZLMRTCClient.Events.WEBRTC_OFFER_ANWSER_EXCHANGE_FAILED, function (e) {// offer anwser 交换失败
          console.log('offer anwser 交换失败', e);
          this.$message({
            message: "webrtc推流失败，请联系管理人员",
            type: "error",
          });
          _that.stopTalk();
        });

        _that.zlmRTCClient.on(ZLMRTCClient.Events.WEBRTC_ON_LOCAL_STREAM, function (s) {// 获取到了本地流
        ...
        _that.zlmRTCClient.on(ZLMRTCClient.Events.CAPTURE_STREAM_FAILED, function (s) {// 获取本地流失败

          console.log('获取本地流失败')
        });

        _that.zlmRTCClient.on(ZLMRTCClient.Events.WEBRTC_ON_CONNECTION_STATE_CHANGE, function (state) {// RTC 状态变化 ,详情参考 https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/connectionState
          console.log('当前状态==>', state)
          if (state === "connected") {
            //推流成功开始下发语音对讲指令
            _that.$axios({
              method: 'get',
              url: `/api/talkback/start/${_that.deviceId}/${_that.channelId}/${app}/${stream}`
            }).then(res => {
              if (res.data.code != 0) {
                this.$message({
                  message: res.data.msg,
              ...
              //console.log(this.el-icon-microphone);
        ...
        _that.zlmRTCClient.on(ZLMRTCClient.Events.WEBRTC_ON_DATA_CHANNEL_OPEN, function (event) {
          console.log('rtc datachannel 打开 :', event)
        });

        _that.zlmRTCClient.on(ZLMRTCClient.Events.WEBRTC_ON_DATA_CHANNEL_ERR, function (event) {
          console.log('rtc datachannel 错误 :', event)
        });

        _that.zlmRTCClient.on(ZLMRTCClient.Events.WEBRTC_ON_DATA_CHANNEL_CLOSE, function (event) {
    ...
    stopTalk() {
      ...
      if (this.zlmRTCClient) {
        ...
        this.$axios({
          method: 'get',
          url: `/api/talkback/stop/${this.deviceId}/${this.channelId}`
        }).then(res => {
          this.$message({
            message: "停止对讲",
            type: "success",
